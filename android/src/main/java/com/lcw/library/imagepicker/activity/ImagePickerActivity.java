...
import com.example.flutter_imgae_picker.R;
...
//import com.example.flutterimagepickerplugin_example.R;

//import com.example.flutter_image_picker_plugin_example.R;
...
public class ImagePickerActivity extends BaseActivity implements ImagePickerAdapter.OnItemClickListener, ImageFoldersAdapter.OnImageFolderChangeListener {
    ...
    protected void initConfig() {
        mButtonColor = ConfigManager.getInstance().getChangeButtonColor();
        mColor = ConfigManager.getInstance().getChangeColor();
    ...
    protected void initView() {
        ...
        //颜色修改
        mRelativeLayout = findViewById(R.id.rl_main_bottom);
        View layoutBottom = findViewById(R.id.layout_actionBar);
        if(TextUtils.isEmpty(mColor)){
            mColor = "#303030";
        }

        if(TextUtils.isEmpty(mButtonColor)){
            mButtonColor = "#44BF1A";
        }
        mRelativeLayout.setBackgroundColor(Color.parseColor(mColor));
        layoutBottom.setBackgroundColor(Color.parseColor(mColor));

        //按钮颜色修改
        mTvCommit.setBackgroundColor(Color.parseColor(mButtonColor));
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (!mediaFolderList.isEmpty()) {
                        //默认加载全部照片
                        mediaFolderList.get(0).getMediaFileList();


                        //如果获取到的是空路径，则不预览显示。
                        ArrayList<MediaFile> Arraylist1 = mediaFolderList.get(0).getMediaFileList();
                        for(int i=0;i<mediaFolderList.get(0).getMediaFileList().size();i++){
                            MediaFile mediaFile = Arraylist1.get(i);
...
//                        mMediaFileList.addAll(mediaFolderList.get(0).getMediaFileList());
                        ...
                        mImageFolderPopupWindow = new ImageFolderPopupWindow(com.lcw.library.imagepicker.activity.ImagePickerActivity.this, mMediaFolderList);
                        mImageFolderPopupWindow.setAnimationStyle(R.style.imageFolderAnimator);
                        mImageFolderPopupWindow.getAdapter().setOnImageFolderChangeListener(com.lcw.library.imagepicker.activity.ImagePickerActivity.this);
